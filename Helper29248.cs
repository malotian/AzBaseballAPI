            ...
            if (conn != null && conn.State == System.Data.ConnectionState.Open) {
                using (SqlCommand cmd = new SqlCommand()) {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader()) {
                        while (reader.Read()) {
                            instructors.Add(new {
                                pk_instructor = reader["pk_instructor"],
                                instructor_first_name = reader["instructor_first_name"],
                                instructor_last_name = reader["instructor_last_name"],
                                expertise = reader["expertise"],
                                instructor_bio = reader["instructor_bio"],
                                general_availability = reader["general_availability"]
                            });
                        }
                    }
                }
